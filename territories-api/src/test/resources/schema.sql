...
create table country (id uuid default uuid_generate_v1() not null, latitude double check (latitude<=90 AND latitude>=-90), longitude double check (longitude>=-180 AND longitude<=180), iso_code2 varchar(2) not null, iso_code3 varchar(3), iso_numeric integer, internet_cctld varchar(5), phone_prefix smallint, continent_id bigint, primary key (id));
drop table if exists country_continents cascade;
create table country_continents (countries_id uuid default uuid_generate_v1() not null, continents_id bigint not null);
...
create table locality (id uuid default uuid_generate_v1() not null, latitude double check (latitude<=90 AND latitude>=-90), longitude double check (longitude>=-180 AND longitude<=180), iso_code2 varchar(2) not null, iso_code3 varchar(3), iso_numeric integer, division_level smallint, local_code varchar(255), name varchar(255), country_id uuid default uuid_generate_v1() not null, parent_locality_id uuid default uuid_generate_v1() not null, primary key (id));
